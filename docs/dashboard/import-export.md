...
4. Under **Sync options**, choose what to sync:
   ...
   - **Run post-sync actions.** Runs any [post-sync actions](docs://core/cli-command/#wp-altis-post-sync)
     ...
     domain root unless a [post-sync URL](./post-sync-url.md) is configured. Subdirectory multisite installs
     need this when their main site is not at the domain root.
